 ...
 * seed-firebase.mjs
 *
 * One-time script to populate Firestore with real portfolio data.
 * Run:  node seed-firebase.mjs
 *
 * Uses the Firebase Web SDK (v9+) directly — no service-account key needed,
 * just the same env vars the site uses (loaded from .env via dotenv).
 *
...
const experiences = [
  {
    ...
    startDate: ts("2025-06-01"),
    endDate: ts("2025-09-30"),
    descriptionPoints: [
  ...
  },
  {
    ...
    location: "Montréal, Canada",
    startDate: ts("2024-05-01"),
    endDate: ts("2025-08-31"),
    descriptionPoints: [
  ...
  },
  {
    ...
    startDate: ts("2023-12-01"),
    endDate: ts("2024-02-28"),
    descriptionPoints: [
  ...
  },
  {
    ...
    company: "Gamefam",
    location: "Edmonton, Canada",
    startDate: ts("2023-03-01"),
    endDate: ts("2023-09-30"),
    descriptionPoints: [
...
async function seed() {
  ...
  const collections = [
    ...
    { name: "hackathons", data: hackathons },
    { name: "experiences", data: experiences },
    { name: "leadership", data: leadership },
  ...
  for (const { name, data } of collections) {
    ...
    console.log(`  → ${name}: ${data.length} docs written\n`);
  }
